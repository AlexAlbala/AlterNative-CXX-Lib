...
public:
   ...
   *   PSocket instances.  If you execute this on Windows while some instance of
   *   PSocket exists, you are toast.  For portability of client code, this is 
...
private:
  ...
  PSocket(const PSocket &sock);
  void operator=(const PSocket &sock);

protected:
  int sockDesc;              // PSocket descriptor
  PSocket(int type, int protocol) throw(SocketException);
  PSocket(int sockDesc);
};

/**
 *   PSocket which is able to connect, send, and receive
 */
class CommunicatingSocket : public PSocket {
public:
  /**
 ...
 */
class TCPServerSocket : public PSocket {
public:
  /**
